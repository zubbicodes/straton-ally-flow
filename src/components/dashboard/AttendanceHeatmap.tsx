  ...
  return (
    <div className="card-elevated p-3 md:p-4 rounded-lg">
      ...
      <div className="flex items-center gap-2 mb-3">
        <span className="text-xl font-display font-bold">{rate}%</span>
        <span className="flex items-center gap-0.5 text-[10px] text-success font-medium bg-success/10 px-1.5 py-0.5 rounded">
      ...
      <div className="flex gap-2">
        ...
        <div className="flex flex-col justify-between py-0.5">
          {hours.map((hour) => (
            <span key={hour} className="text-[9px] text-muted-foreground leading-none">{hour}</span>
          ))}
        </div>
        ...
        <div className="flex-1">
            ...
            {heatmapData.map((row, rowIndex) => (
              row.map((intensity, colIndex) => (
                <div
                  ...
                  className={cn(
                    'h-5 md:h-6 rounded-sm transition-colors',
          ...
          <div className="grid grid-cols-7 gap-0.5 mt-1">
            {days.map((day) => (
              <span key={day} className="text-[9px] text-muted-foreground text-center">
                {day}
              </span>
            ))}
          </div>
