...
export function AttendanceSystem() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          in_time: now,
          status,
          ip_address: locationInfo.currentIP
        })
        .select()
        .single();
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      setAttendance(prev => prev ? { ...prev, out_time: now, total_hours: totalHoursStr } : null);
      toast({
  ...
  const handleBreakStart = async () => {
    ...
    try {
      ...
      setAttendance(prev => prev ? { ...prev, break_start: now } : null);
      toast({
        title: "Break Started",
        description: `Break started at ${now}`,
      });
    } catch (error) {
  ...
  const handleBreakEnd = async () => {
    ...
    try {
      ...
      setAttendance(prev => prev ? { ...prev, break_end: now } : null);
      toast({
        title: "Break Ended",
        description: `Break ended at ${now}`,
      });
    } catch (error) {
            ...
            <div>
              ...
              <p className="font-medium">
                {attendance?.break_start && attendance?.break_end ? 
                  `${attendance.break_start} - ${attendance.break_end}` : 
                  attendance?.break_start ? `${attendance.break_start} - Ongoing` : '—'
                }
              </p>
            </div>
            <div>
              ...
              <>
                ...
                {!attendance?.break_start ? (
                  <Button 
                    ...
                    disabled={!attendance?.in_time || attendance?.out_time || isLoading}
                    className="flex items-center gap-2"
                  >
